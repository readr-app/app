...
const getProgressBar = () => {
    const outer = classnames(
        styles.bar,
        mdlprogress['mdl-progress'],
        mdlprogress['mdl-progress__indeterminate']
    );
    const progress = classnames(mdlprogress.progressbar, mdlprogress.bar, mdlprogress.bar1);
    const buffer = classnames(mdlprogress.bufferbar, mdlprogress.bar, mdlprogress.bar2);
    const aux = classnames(mdlprogress.auxbar, mdlprogress.bar, mdlprogress.bar3);
    return (
        <div className={outer}>
...
const Progress = props => {
    ...
    if (isLoading) {
        return (
            <div className={styles.progress} id={ID_LOADING}>
                {getProgressBar()}
    ...
    if (success) {
        return (
            <div className={styles.progress} id={ID_SUCCESS}>
                <span className={styles.caption}>Article successfully stored. Happy reading!</span>
            </div>
        );
    }
    ...
    if (errorTxt) {
        return (
            <div className={styles.progress} id={ID_ERROR}>
