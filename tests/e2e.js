...
fixture('Home').page(`http://localhost:${process.env.PORT || 8090}`);
...
const sourceOne = {
    url:
        'http://www.volksstimme.de/lokal/schoenebeck/kreis-gegen-ameos-erster-streit-nun-beigelegt',
    color: 'rgb(63, 81, 181)',
    title: 'Erster Streit nun beigelegt',
};
const sourceTwo = {
    url:
        'http://www.spiegel.de/politik/ausland/john-lewis-buergerrechtler-bezeichnet-trump-als-illegitimen-praesidenten-a-1130038.html',
    color: 'rgb(153, 0, 0)',
...
const addArticle = async (t, { url, title }) => {
    await t.typeText(input, url).pressKey('enter');
    ...
    await t.expect(listItem.exists).ok().expect(listItem.textContent).contains(title);
};
...
test('Successfully add article', async t => addArticle(t, sourceOne));

test('Successfully add another article', async t => addArticle(t, sourceTwo));
...
test('Delete article', async t => {
    ...
    const listItemCount = await t.eval(() => document.querySelectorAll('#article-list li').length);
